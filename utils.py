...
def extract_bboxes(mask):
    ...
    """
    boxes = torch.zeros([mask.shape[0], 4], dtype=torch.int32)
    for i in range(mask.shape[0]):
        ...
        if horizontal_indicies.shape[0]:
            ...
            x1, x2, y1, y2 = 0, 0, 0, 0
        boxes[i] = torch.tensor([y1, x1, y2, x2], dtype=torch.int32)
    return boxes
...
class Dataset(object):
    ...
    def load_image(self, image_id):
        ...
        """
        # Load image
        image = Image.open(self.image_info[image_id]['path']).convert('RGB')
